...
export function hBarChart(
  ...
  const accent = opts.accent ?? "#2dd4bf";
  ...
  const rows = bars
    .map((b, i) => {
      ...
      return [
        `<text x="0" y="${y + BAR_H - 6}" font-size="12" fill="#8b96a8">${escapeXml(b.label)}</text>`,
        `<rect x="${LABEL_W}" y="${y}" width="${barArea}" height="${BAR_H}" rx="3" fill="#1b2330" />`,
        `<rect x="${LABEL_W}" y="${y}" width="${w}" height="${BAR_H}" rx="3" fill="${accent}" />`,
        `<text x="${width}" y="${y + BAR_H - 6}" font-size="12" text-anchor="end" fill="#e6e9ee" style="font-variant-numeric: tabular-nums">${escapeXml(b.display)}</text>`,
      ].join("");
    })
    .join("\n");
...
export function sparkline(
  ...
  const pad = 8;
  if (points.length < 2) {
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" role="img"><text x="${pad}" y="${height / 2}" font-size="11" fill="#8b96a8">not enough data points</text></svg>`;
  }
  ...
  const dots = [coords[0]!, coords[coords.length - 1]!]
    ...
    .join("");
  const labels = `<text x="${pad}" y="${height - 1}" font-size="9" fill="#8b96a8">${escapeXml(points[0]!.label)}</text>` +
    `<text x="${width - pad}" y="${height - 1}" font-size="9" text-anchor="end" fill="#8b96a8">${escapeXml(points[points.length - 1]!.label)}</text>`;
